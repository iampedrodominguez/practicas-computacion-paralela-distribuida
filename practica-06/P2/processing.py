...
exp = 10
for size in sizes:
    plt.plot(processes, times_dict[size]['parallel'], color=colors[sizes.index(size)], label='Par (2^{})'.format(exp))
    plt.plot(processes, times_dict[size]['odd_even'], color=colors[sizes.index(size)], linestyle='--', label='O-E (2^{})'.format(exp))
    exp += 1

# legend transparent
plt.legend(ncol=5, loc='upper left', prop={'size': 7}, framealpha=0.1)
plt.title('Times')
...
for size in sizes:
    ...
    plt.plot(processes, times_dict[size]['speedup_parallel'], color=colors[sizes.index(size)], label='Par(2^{})'.format(exp))
    plt.plot(processes, times_dict[size]['speedup_odd_even'], color=colors[sizes.index(size)], linestyle='--', label='O-E(2^{})'.format(exp))
    exp += 1

plt.legend(ncol=5, loc='upper right', prop={'size': 7}, framealpha=0.1)
plt.title('Speedup')
plt.xlabel('Processes')
plt.ylabel('Speedup')
plt.xscale('log', base=2)
